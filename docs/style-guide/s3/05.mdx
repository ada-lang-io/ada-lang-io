...
- Use underscores to separate digits the same way commas or periods (or
  spaces for nondecimal bases) would be used in normal text.
...
- To justify its use, an abbreviation must save many characters over the
  full word.
...
- Structure comments in headers so that information can be automatically
  extracted by a tool.
