...
Manual (1995, §§[Ada Reference Manual, §7.1](../../arm/AA-7/AA-7.1.mdx),
[Ada Reference Manual, §7.2](../../arm/AA-7/AA-7.2.mdx), and [Ada Reference
Manual, §10.1.1](../../arm/AA-10/AA-10.1.mdx#Subclause_10.1.1)):
...
way. In the example above, it is clear that the package `Text_IO` is
used only by the `Listing_Facilities` package of the compiler.
...
One way to minimize the coverage of a `with` clause is to use it only
with subunits that really need it. Consider making those subunits
separate compilation units when the need for visibility to a library
unit is restricted to a subprogram or two.
