...
<strong></strong>The contents of the visible part of package Interfaces and its language-defined descendants.<br />
</Admonition>
</AnnotatedOnly>
...
<ul>
<li>Signed and modular integer types of <em>n</em> bits, if supported by the target architecture, for each <em>n</em> that is at least the size of a storage element and that is a factor of the word size. The names of these types are of the form Integer{"_"}<em>n</em> for the signed types, and Unsigned{"_"}<em>n</em> for the modular types; <br /></li>
</ul>
<AnnotatedOnly>
<MarginText>8.a</MarginText>
<Admonition type="aarm" aarm="ramification">
<strong></strong>For example, for a typical 32-bit machine the corresponding types might be Integer{"_"}8, Unsigned{"_"}8, Integer{"_"}16, Unsigned{"_"}16, Integer{"_"}32, and Unsigned{"_"}32.<br />
</Admonition>
</AnnotatedOnly>
...
<strong></strong>The names for these floating point types are not specified. However, if IEEE arithmetic is supported, then the names should be IEEE{"_"}Float{"_"}32 and IEEE{"_"}Float{"_"}64 for single and double precision, respectively.<br />
</Admonition>
</AnnotatedOnly>
...
<Admonition type="aarm" aarm="implementation-defined">
<strong></strong>Implementation-defined children of package Interfaces.<br />
</Admonition>
</AnnotatedOnly>
...
<Admonition type="aarm" aarm="ramification">
<strong></strong>For example, package Interfaces.XYZ{"_"}Pascal might contain declarations of types that match the data types provided by the XYZ implementation of Pascal, so that it will be more convenient to pass parameters to a subprogram whose convention is XYZ{"_"}Pascal. <br />
</Admonition>
</AnnotatedOnly>
...
<strong></strong>If an interface to C, COBOL, or Fortran is provided, the corresponding package or packages described in <a href="/docs/arm/AA-B/">Annex B</a>, &ldquo;<a href="/docs/arm/AA-B/">Interface to Other Languages</a>&rdquo; should also be provided.<br />
</Admonition>
</AnnotatedOnly>
...
<Admonition type="aarm" aarm="implementation-note">
<strong></strong>The intention is that an implementation might support several implementations of the foreign language: Interfaces.This{"_"}Fortran and Interfaces.That{"_"}Fortran might both exist. The &ldquo;default&rdquo; implementation, overridable by the user, should be declared as a renaming: <br />
</Admonition>
</AnnotatedOnly>
