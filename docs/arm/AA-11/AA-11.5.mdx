...
<strong>Term entry: </strong><strong>suppress a check</strong> &mdash; assert that the check cannot fail, and request that the compiler optimize by disabling the check<br /><br /></Admonition>
</AnnotatedOnly>
