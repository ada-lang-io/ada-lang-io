...
<MarginInfo items={["AI05-0299-1"]} /><p>{"{"}<em>{"AI05-0299-1"}</em>{"}"} <br />[ This subclause gives permission to the implementation to perform certain &ldquo;optimizations&rdquo; that do not necessarily preserve the canonical semantics.] <br /></p>


#### Dynamic Semantics

<MarginText>2/3</MarginText>
<MarginInfo items={["AI05-0299-1"]} /><p>{"{"}<em>{"AI05-0299-1"}</em>{"}"} <br />The rest of this Reference Manual (outside this subclause) defines the <em>canonical semantics</em> of the language. [The canonical semantics of a given (legal) program determines a set of possible external effects that can result from the execution of the program with given inputs.] <br /></p>
...
<MarginInfo items={["AI05-0229-1", "AI12-0445-1"]} /><ul>
...
<Admonition type="aarm" aarm="reason" title="Reason: ">
<strong></strong>We allow such variables to become abnormal so that assignments (other than to atomic variables) can be disrupted due to &ldquo;imprecise&rdquo; exceptions or instruction scheduling, and so that assignments can be reordered so long as the correct results are produced in the end if no language-defined checks fail. <br /></Admonition>
...
<MarginInfo items={["AI05-0299-1", "AI12-0005-1"]} /><Admonition type="aarm" aarm="note" title="Note: ">
{"{"}<em>{"AI05-0299-1"}</em>{"}"} {"{"}<em>{"AI12-0005-1"}</em>{"}"} <br />We moved subclause <a href="../AA-11/AA-11.5">11.5</a>, &ldquo;<a href="../AA-11/AA-11.5">Suppressing Checks</a>&rdquo; (which was numbered 11.7) from after 11.6 to before 11.6, in order to preserve the famous number &ldquo;11.6&rdquo; (given the changes to earlier subclauses in Clause <a href="../AA-11/">11</a>). <br /></Admonition>
