...
<strong></strong>This follows (and echos) the standard rule of interfacing; the programmer must ensure that Ada semantics are followed (see <a href="/docs/arm/AA-B/AA-B.1">B.1</a>). <br />
</Admonition>
</AnnotatedOnly>
...
<MarginInfo items={["AI95-00426-01"]} />
<Admonition type="aarm" aarm="implementation-note">
<strong></strong>This means that the implementation must take care not to use an invalid representation in a way that might cause erroneous execution. For instance, the exception mandated for <code><a href="/docs/arm/AA-5/AA-5.4#S0176">case_statement</a></code>s must be raised. Array indexing must not cause memory outside of the array to be written (and usually, not read either). These cases and similar cases may require explicit checks by the implementation. <br />
</Admonition>
</AnnotatedOnly>
...
<MarginInfo items={["AI12-0071-1"]} />
<Admonition type="aarm" aarm="note">
<strong>Corrigendum:</strong> Updated wording of the attributes X'Valid to use the new term "satisfies the predicates" (see <a href="/docs/arm/AA-3/AA-3.2#Subclause_3.2.4">3.2.4</a>). Also updated the notes to make sense when evaluating predicates and testing validity of volatile objects. <br />
</Admonition>
</AnnotatedOnly>
