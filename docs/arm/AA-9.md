...
The properties of a task are defined by a corresponding task declaration and [task_body](S0179), which together define a program unit called a task unit. 
...
A task unit is declared by a task declaration, which has a corresponding [task_body](S0179). A task declaration may be a [task_type_declaration](S0175), in which case it declares a named task type; alternatively, it may be a [single_task_declaration](S0176), in which case it defines an anonymous task type, as well as declaring a named task object of that type. 
...
If a task_[identifier](S0002) appears at the end of a [task_definition](S0177) or [task_body](S0179), it shall repeat the [defining_identifier](S0019). 


#### Legality Rules

A task declaration requires a completion[, which shall be a [task_body](S0179),] and every [task_body](S0179) shall be the completion of some task declaration. 

To be honest: The completion can be a [pragma](S0016) Import, if the implementation supports it. 


#### Static Semantics

A [task_definition](S0177) defines a task type and its first subtype. The first list of [task_item](S0178)s of a [task_definition](S0177), together with the [known_discriminant_part](S0058), if any, is called the visible part of the task unit. [ The optional list of [task_item](S0178)s after the reserved word private is called the private part of the task unit.] 
...
[ The elaboration of a task declaration elaborates the [task_definition](S0177). The elaboration of a [single_task_declaration](S0176) also creates an object of an (anonymous) task type.] 

Proof: This is redundant with the general rules for the elaboration of a [full_type_declaration](S0021) and an [object_declaration](S0029). 

[The elaboration of a [task_definition](S0177) creates the task type and its first subtype;] it also includes the elaboration of the [entry_declaration](S0187)s in the given order.

As part of the initialization of a task object, any [representation_clause](S0263)s and any per-object constraints associated with [entry_declaration](S0187)s of the corresponding [task_definition](S0177) are elaborated in the given order. 

Reason: The only [representation_clause](S0263)s defined for task entries are ones that specify the Address of an entry, as part of defining an interrupt entry. These clearly need to be elaborated per-object, not per-type. Normally the address will be a function of a discriminant, if such an Address clause is in a task type rather than a single task declaration, though it could rely on a parameterless function that allocates sequential interrupt vectors.

We do not mention representation pragmas, since each pragma may have its own elaboration rules. 

The elaboration of a [task_body](S0179) has no effect other than to establish that tasks of the type can from then on be activated without failing the Elaboration_Check.

[The execution of a [task_body](S0179) is invoked by the activation of a task of the corresponding type (see 9.2).]
...
NOTE 1   Within the declaration or body of a task unit, the name of the task unit denotes the current instance of the unit (see 8.6), rather than the first subtype of the corresponding task type (and thus the name cannot be used as a [subtype_mark](S0025)). 

Discussion: However, it is possible to refer to some other subtype of the task type within its body, presuming such a subtype has been declared between the [task_type_declaration](S0175) and the [task_body](S0179). 

NOTE 2   The notation of a [selected_component](S0091) can be used to denote a discriminant of a task (see 4.1.3). Within a task unit, the name of a discriminant of the task type denotes the corresponding discriminant of the current instance of the unit.
...
The syntax rules for task declarations are modified to allow a [known_discriminant_part](S0058), and to allow a private part. They are also modified to allow [entry_declaration](S0187)s and [representation_clause](S0263)s to be mixed. 


#### Wording Changes from Ada 83

The syntax rules for tasks have been split up according to task types and single tasks. In particular: The syntax rules for task_declaration and task_specification are removed. The syntax rules for [task_type_declaration](S0175), [single_task_declaration](S0176), [task_definition](S0177) and [task_item](S0178) are new.

The syntax rule for [task_body](S0179) now uses the nonterminal [handled_sequence_of_statements](S0231).

The [declarative_part](S0079) of a [task_body](S0179) is now required; that doesn't make any real difference, because a [declarative_part](S0079) can be empty. 
...
The execution of a task of a given task type consists of the execution of the corresponding [task_body](S0179). The initial part of this execution is called the activation of the task; it consists of the elaboration of the [declarative_part](S0079) of the [task_body](S0179). Should an exception be propagated by the elaboration of its [declarative_part](S0079), the activation of the task is defined to have failed, and it becomes a completed task.

A task object (which represents one task) can be created either as part of the elaboration of an [object_declaration](S0029) occurring immediately within some declarative region, or as part of the evaluation of an [allocator](S0122). All tasks created by the elaboration of [object_declaration](S0029)s of a single declarative region (including subcomponents of the declared objects) are activated together. Similarly, all tasks created by the evaluation of a single [allocator](S0122) are activated together. The activation of a task is associated with the innermost [allocator](S0122) or [object_declaration](S0029) that is responsible for its creation. 

Discussion: The initialization of an [object_declaration](S0029) or [allocator](S0122) can indirectly include the creation of other objects that contain tasks. For example, the default expression for a subcomponent of an object created by an [allocator](S0122) might call a function that evaluates a completely different [allocator](S0122). Tasks created by the two allocators are not activated together. 

For tasks created by the elaboration of [object_declaration](S0029)s of a given declarative region, the activations are initiated within the context of the [handled_sequence_of_statements](S0231) (and its associated [exception_handler](S0232)s if any - see 11.2), just prior to executing the statements of the _sequence. [For a package without an explicit body or an explicit [handled_sequence_of_statements](S0231), an implicit body or an implicit [null_statement](S0127) is assumed, as defined in 7.2.] 

Ramification: If Tasking_Error is raised, it can be handled by handlers of the [handled_sequence_of_statements](S0231). 

For tasks created by the evaluation of an [allocator](S0122), the activations are initiated as the last step of evaluating the [allocator](S0122), after completing any initialization for the object created by the [allocator](S0122), and prior to returning the new access value.
...
Ramification: Note that a task created by an [allocator](S0122) does not necessarily depend on its activator; in such a case the activator's termination can precede the termination of the newly created task. 
...
If the task is created by the evaluation of an [allocator](S0122) for a given access type, it depends on each master that includes the elaboration of the declaration of the ultimate ancestor of the given access type.

If the task is created by the elaboration of an [object_declaration](S0029), it depends on each master that includes this elaboration.
...
A task is said to be completed when the execution of its corresponding [task_body](S0179) is completed. A task is said to be terminated when any finalization of the [task_body](S0179) has been performed (see 7.6.1). [The first step of finalizing a master (including a [task_body](S0179)) is to wait for the termination of any tasks dependent on the master.] The task executing the master is blocked until all the dependents have terminated. [Any remaining finalization is then performed and the master is left.]

Completion of a task (and the corresponding [task_body](S0179)) can occur when the task is blocked at a [select_statement](S0199) with an an open [terminate_alternative](S0205) (see 9.7.1); the open [terminate_alternative](S0205) is selected if and only if the following conditions are satisfied: 

The task depends on some completed master;

Each task that depends on the master considered is either already terminated or similarly blocked at a [select_statement](S0199) with an open [terminate_alternative](S0205). 
...
Ramification: Any required finalization is performed after the selection of [terminate_alternative](S0205)s. The tasks are not callable during the finalization. In some ways it is as though they were aborted. 
...
NOTE 2   An [object_renaming_declaration](S0170) defines a new view of an existing entity and hence creates no further dependence.

NOTE 3   The rules given for the collective completion of a group of tasks all blocked on [select_statement](S0199)s with open [terminate_alternative](S0205)s ensure that the collective completion can occur only when there are no remaining active tasks that could call one of the tasks being collectively completed.

NOTE 4   If two or more tasks are blocked on [select_statement](S0199)s with open [terminate_alternative](S0205)s, and become completed collectively, their finalization actions proceed concurrently.

NOTE 5   The completion of a task can occur due to any of the following: 

the raising of an exception during the elaboration of the [declarative_part](S0079) of the corresponding [task_body](S0179);

the completion of the [handled_sequence_of_statements](S0231) of the corresponding [task_body](S0179);

the selection of an open [terminate_alternative](S0205) of a [select_statement](S0199) in the corresponding [task_body](S0179);
...
Tasks that used to depend on library packages in Ada 83, now depend on the (implicit) [task_body](S0179) of the environment task (see 10.2). Therefore, the environment task has to wait for them before performing library level finalization and terminating the partition. In Ada 83 the requirement to wait for tasks that depended on library packages was not as clear.

What was "collective termination" is now "collective completion" resulting from selecting [terminate_alternative](S0205)s. This is because finalization still occurs for such tasks, and this happens after selecting the [terminate_alternative](S0205), but before termination. 


## 9.4  Protected Units and Protected Objects

A protected object provides coordinated access to shared data, through calls on its visible protected operations, which can be protected subprograms or protected entries. A protected unit is declared by a protected declaration, which has a corresponding [protected_body](S0185). A protected declaration may be a [protected_type_declaration](S0180), in which case it declares a named protected type; alternatively, it may be a [single_protected_declaration](S0181), in which case it defines an anonymous protected type, as well as declaring a named protected object of that type. 
...
Reason: We allow the operations and components to be mixed because that's how other things work (for example, package declarations). We have relaxed the ordering rules for the items inside [declarative_part](S0079)s and [task_definition](S0177)s as well. 
...
If a protected_[identifier](S0002) appears at the end of a [protected_definition](S0182) or [protected_body](S0185), it shall repeat the [defining_identifier](S0019). 


#### Legality Rules

A protected declaration requires a completion[, which shall be a [protected_body](S0185),] and every [protected_body](S0185) shall be the completion of some protected declaration. 
...
A [protected_definition](S0182) defines a protected type and its first subtype. The list of [protected_operation_declaration](S0183)s of a [protected_definition](S0182), together with the [known_discriminant_part](S0058), if any, is called the visible part of the protected unit. [ The optional list of [protected_element_declaration](S0184)s after the reserved word private is called the private part of the protected unit.] 
...
[The elaboration of a protected declaration elaborates the [protected_definition](S0182). The elaboration of a [single_protected_declaration](S0181) also creates an object of an (anonymous) protected type.] 

Proof: This is redundant with the general rules for the elaboration of a [full_type_declaration](S0021) and an [object_declaration](S0029). 

[The elaboration of a [protected_definition](S0182) creates the protected type and its first subtype;] it also includes the elaboration of the [component_declaration](S0067)s and [protected_operation_declaration](S0183)s in the given order.
...
The elaboration of a [protected_body](S0185) has no other effect than to establish that protected operations of the type can from then on be called without failing the Elaboration_Check.
...
As the first step of the finalization of a protected object, each call remaining on any entry queue of the object is removed from its queue and Program_Error is raised at the place of the corresponding [entry_call_statement](S0194). 
...
NOTE 1   Within the declaration or body of a protected unit, the name of the protected unit denotes the current instance of the unit (see 8.6), rather than the first subtype of the corresponding protected type (and thus the name cannot be used as a [subtype_mark](S0025)). 

Discussion: However, it is possible to refer to some other subtype of the protected type within its body, presuming such a subtype has been declared between the [protected_type_declaration](S0180) and the [protected_body](S0185). 

NOTE 2   A [selected_component](S0091) can be used to denote a discriminant of a protected object (see 4.1.3). Within a protected unit, the name of a discriminant of the protected type denotes the corresponding discriminant of the current instance of the unit.
...
NOTE 4   The bodies of the protected operations given in the [protected_body](S0185) define the actions that take place upon calls to the protected operations.

NOTE 5   The declarations in the private part are only visible within the private part and the body of the protected unit. 

Reason: [Component_declaration](S0067)s are disallowed in a [protected_body](S0185) because, for efficiency, we wish to allow the compiler to determine the size of protected objects (when not dynamic); the compiler cannot necessarily see the body. Furthermore, the semantics of initialization of such objects would be problematic - we do not wish to give protected objects complex initialization semantics similar to task activation.

The same applies to [entry_declaration](S0187)s, since an entry involves an implicit component - the entry queue. 
...
Any call on an entry or on a protected subprogram identifies a target object for the operation, which is either a task (for an entry call) or a protected object (for an entry call or a protected subprogram call). The target object is considered an implicit parameter to the operation, and is determined by the operation [name](S0084) (or [prefix](S0086)) used in the call on the operation, as follows: 

If it is a [direct_name](S0085) or expanded name that denotes the declaration (or body) of the operation, then the target object is implicitly specified to be the current instance of the task or protected unit immediately enclosing the operation; such a call is defined to be an internal call;

If it is a [selected_component](S0091) that is not an expanded name, then the target object is explicitly specified to be the task or protected object denoted by the [prefix](S0086) of the [name](S0084); such a call is defined to be an external call; 
...
If the [name](S0084) or [prefix](S0086) is a dereference (implicit or explicit) of an access-to-protected-subprogram value, then the target object is determined by the [prefix](S0086) of the Access [attribute_reference](S0093) that produced the access value originally, and the call is defined to be an external call;

If the [name](S0084) or [prefix](S0086) denotes a [subprogram_renaming_declaration](S0173), then the target object is as determined by the [name](S0084) of the renamed entity.

A corresponding definition of target object applies to a [requeue_statement](S0195) (see 9.5.4), with a corresponding distinction between an internal requeue and an external requeue.
...
To be honest: The current instance is defined in the same way within the body of a subprogram declared immediately within a [protected_body](S0185). 
...
A protected subprogram is a subprogram declared immediately within a [protected_definition](S0182). Protected procedures provide exclusive read-write access to the data of a protected object; protected functions provide concurrent read-only access to the data. 

Ramification: A subprogram declared immediately within a [protected_body](S0185) is not a protected subprogram; it is an intrinsic subprogram. See 6.3.1, "Conformance Rules". 


#### Static Semantics

[Within the body of a protected function (or a function declared immediately within a [protected_body](S0185)), the current instance of the enclosing protected unit is defined to be a constant (that is, its subcomponents may be read but not updated). Within the body of a protected procedure (or a procedure declared immediately within a [protected_body](S0185)), and within an [entry_body](S0190), the current instance is defined to be a variable (updating is permitted).] 
...
a [select_statement](S0199);

an [accept_statement](S0188);

an [entry_call_statement](S0194);

a [delay_statement](S0196);

an [abort_statement](S0213);
...
[Entry_declaration](S0187)s, with the corresponding entry_bodies or [accept_statement](S0188)s, are used to define potentially queued operations on tasks and protected objects. 
...
Reason: We cannot use [defining_identifier](S0019) for [accept_statement](S0188)s. Although an [accept_statement](S0188) is sort of like a body, it can appear nested within a [block_statement](S0138), and therefore be hidden from its own entry by an outer homograph. 
...
If an entry_[identifier](S0002) appears at the end of an [accept_statement](S0188), it shall repeat the entry_[direct_name](S0085). If an entry_[identifier](S0002) appears at the end of an [entry_body](S0190), it shall repeat the [defining_identifier](S0019).

[An [entry_declaration](S0187) is allowed only in a protected or task declaration.] 
...
In an [accept_statement](S0188), the expected profile for the entry_[direct_name](S0085) is that of the [entry_declaration](S0187); the expected type for an [entry_index](S0189) is that of the subtype defined by the [discrete_subtype_definition](S0052) of the corresponding [entry_declaration](S0187).

Within the [handled_sequence_of_statements](S0231) of an [accept_statement](S0188), if a [selected_component](S0091) has a [prefix](S0086) that denotes the corresponding [entry_declaration](S0187), then the entity denoted by the [prefix](S0086) is the [accept_statement](S0188), and the [selected_component](S0091) is interpreted as an expanded name (see 4.1.3)[; the [selector_name](S0092) of the [selected_component](S0091) has to be the [identifier](S0002) for some formal parameter of the [accept_statement](S0188)]. 

Proof: The only declarations that occur immediately within the declarative region of an [accept_statement](S0188) are those for its formal parameters. 


#### Legality Rules

An [entry_declaration](S0187) in a task declaration shall not contain a specification for an access parameter (see 3.10). 
...
Implementing the Accessibility_Check inside the [accept_statement](S0188) for E is difficult, since one does not know whether the entry caller is calling from inside the immediately enclosing declare block or from outside it. This means that the lexical nesting level associated with the designated object is not sufficient to determine whether the Accessibility_Check should pass or fail.

Note that such problems do not arise with protected entries, because entry_bodies are always nested immediately within the [protected_body](S0185); they cannot be further nested as can [accept_statement](S0188)s, nor can they be called from within the [protected_body](S0185) (since no entry calls are permitted inside a [protected_body](S0185)). 

For an [accept_statement](S0188), the innermost enclosing body shall be a [task_body](S0179), and the entry_[direct_name](S0085) shall denote an [entry_declaration](S0187) in the corresponding task declaration; the profile of the [accept_statement](S0188) shall conform fully to that of the corresponding [entry_declaration](S0187). An [accept_statement](S0188) shall have a parenthesized [entry_index](S0189) if and only if the corresponding [entry_declaration](S0187) has a [discrete_subtype_definition](S0052).

An [accept_statement](S0188) shall not be within another [accept_statement](S0188) that corresponds to the same [entry_declaration](S0187), nor within an [asynchronous_select](S0209) inner to the enclosing [task_body](S0179). 

Reason: [Accept_statement](S0188)s are required to be immediately within the enclosing [task_body](S0179) (as opposed to being in a nested subprogram) to ensure that a nested task does not attempt to accept the entry of its enclosing task. We considered relaxing this restriction, either by making the check a run-time check, or by allowing a nested task to accept an entry of its enclosing task. However, neither change seemed to provide sufficient benefit to justify the additional implementation burden.

Nested [accept_statement](S0188)s for the same entry (or entry family) are prohibited to ensure that there is no ambiguity in the resolution of an expanded name for a formal parameter of the entry. This could be relaxed by allowing the inner one to hide the outer one from all visibility, but again the small added benefit didn't seem to justify making the change for Ada 95.

[Accept_statement](S0188)s are not permitted within [asynchronous_select](S0209) statements to simplify the semantics and implementation: an [accept_statement](S0188) in an [abortable_part](S0212) could result in Tasking_Error being propagated from an entry call even though the target task was still callable; implementations that use multiple tasks implicitly to implement an [asynchronous_select](S0209) might have trouble supporting "up-level" accepts. Furthermore, if [accept_statement](S0188)s were permitted in the [abortable_part](S0212), a task could call its own entry and then accept it in the [abortable_part](S0212), leading to rather unusual and possibly difficult-to-specify semantics. 

An [entry_declaration](S0187) of a protected unit requires a completion[, which shall be an [entry_body](S0190),] and every [entry_body](S0190) shall be the completion of an [entry_declaration](S0187) of a protected unit. The profile of the [entry_body](S0190) shall conform fully to that of the corresponding declaration. 

Ramification: An [entry_declaration](S0187), unlike a [subprogram_declaration](S0141), cannot be completed with a [renaming_declaration](S0169). 

To be honest: The completion can be a [pragma](S0016) Import, if the implementation supports it. 

Discussion: The above applies only to protected entries, which are the only ones completed with entry_bodies. Task entries have corresponding [accept_statement](S0188)s instead of having entry_bodies, and we do not consider an [accept_statement](S0188) to be a "completion," because a task [entry_declaration](S0187) is allowed to have zero, one, or more than one corresponding [accept_statement](S0188)s. 

An [entry_body_formal_part](S0191) shall have an [entry_index_specification](S0193) if and only if the corresponding [entry_declaration](S0187) has a [discrete_subtype_definition](S0052). In this case, the [discrete_subtype_definition](S0052)s of the [entry_declaration](S0187) and the [entry_index_specification](S0193) shall fully conform to one another (see 6.3.1). 

A name that denotes a formal parameter of an [entry_body](S0190) is not allowed within the [entry_barrier](S0192) of the [entry_body](S0190).


#### Static Semantics

The parameter modes defined for parameters in the [parameter_profile](S0149) of an [entry_declaration](S0187) are the same as for a [subprogram_declaration](S0141) and have the same meaning (see 6.2). 

Discussion: Note that access parameters are not allowed for task entries (see above). 

An [entry_declaration](S0187) with a [discrete_subtype_definition](S0052) (see 3.6) declares a family of distinct entries having the same profile, with one such entry for each value of the entry index subtype defined by the [discrete_subtype_definition](S0052). [A name for an entry of a family takes the form of an [indexed_component](S0089), where the [prefix](S0086) denotes the [entry_declaration](S0187) for the family, and the index value identifies the entry within the family.] The term single entry is used to refer to any entry other than an entry of an entry family.

In the [entry_body](S0190) for an entry family, the [entry_index_specification](S0193) declares a named constant whose subtype is the entry index subtype defined by the corresponding [entry_declaration](S0187); the value of the named entry index identifies which entry of the family was called. 

Ramification: The [discrete_subtype_definition](S0052) of the [entry_index_specification](S0193) is not elaborated; the subtype of the named constant declared is defined by the [discrete_subtype_definition](S0052) of the corresponding [entry_declaration](S0187), which is elaborated, either when the type is declared, or when the object is created, if its constraint is per-object. 


#### Dynamic Semantics

For the elaboration of an [entry_declaration](S0187) for an entry family, if the [discrete_subtype_definition](S0052) contains no per-object expressions (see 3.8), then the [discrete_subtype_definition](S0052) is elaborated. Otherwise, the elaboration of the [entry_declaration](S0187) consists of the evaluation of any expression of the [discrete_subtype_definition](S0052) that is not a per-object expression (or part of one). The elaboration of an [entry_declaration](S0187) for a single entry has no effect. 
...
[The actions to be performed when an entry is called are specified by the corresponding [accept_statement](S0188)s (if any) for an entry of a task unit, and by the corresponding [entry_body](S0190) for an entry of a protected unit.]

For the execution of an [accept_statement](S0188), the [entry_index](S0189), if any, is first evaluated and converted to the entry index subtype; this index value identifies which entry of the family is to be accepted. Further execution of the [accept_statement](S0188) is then blocked until a caller of the corresponding entry is selected (see 9.5.3), whereupon the [handled_sequence_of_statements](S0231), if any, of the [accept_statement](S0188) is executed, with the formal parameters associated with the corresponding actual parameters of the selected entry call. of the [handled_sequence_of_statements](S0231), the [accept_statement](S0188) completes and is left. When an exception is propagated from the [handled_sequence_of_statements](S0231) of an [accept_statement](S0188), the same exception is also raised by the execution of the corresponding [entry_call_statement](S0194). 

Ramification: This is in addition to propagating it to the construct containing the [accept_statement](S0188). In other words, for a rendezvous, the raising splits in two, and continues concurrently in both tasks.

The caller gets a new occurrence; this isn't considered propagation.

Note that we say "propagated from the [handled_sequence_of_statements](S0231) of an [accept_statement](S0188)", not "propagated from an [accept_statement](S0188)". The latter would be wrong - we don't want exceptions propagated by the [entry_index](S0189) to be sent to the caller (there is none yet!).
...
[An [entry_body](S0190) is executed when the [condition](S0132) of the [entry_barrier](S0192) evaluates to True and a caller of the corresponding single entry, or entry of the corresponding entry family, has been selected (see 9.5.3).] For the execution of the [entry_body](S0190), the [declarative_part](S0079) of the [entry_body](S0190) is elaborated, and the [handled_sequence_of_statements](S0231) of the body is executed, as for the execution of a [subprogram_body](S0154). The value of the named entry index, if any, is determined by the value of the entry index specified in the entry_[name](S0084) of the selected entry call (or intermediate [requeue_statement](S0195) - see 9.5.4). 

To be honest: If the entry had been renamed as a subprogram, and the call was a [procedure_call_statement](S0155) using the name declared by the renaming, the entry index (if any) comes from the entry [name](S0084) specified in the [subprogram_renaming_declaration](S0173). 
...
NOTE 2   A consequence of the rule regarding the allowed placements of [accept_statement](S0188)s is that a task can execute [accept_statement](S0188)s only for its own entries.

NOTE 3   A [return_statement](S0160) (see 6.5) or a [requeue_statement](S0195) (see 9.5.4) may be used to complete the execution of an [accept_statement](S0188) or an [entry_body](S0190). 

Ramification: An [accept_statement](S0188) need not have a [handled_sequence_of_statements](S0231) even if the corresponding entry has parameters. Equally, it can have a [handled_sequence_of_statements](S0231) even if the corresponding entry has no parameters. 

Ramification: A single entry overloads a subprogram, an enumeration literal, or another single entry if they have the same [defining_identifier](S0019). Overloading is not allowed for entry family names. A single entry or an entry of an entry family can be renamed as a procedure as explained in 8.5.4. 

NOTE 4   The [condition](S0132) in the [entry_barrier](S0192) may reference anything visible except the formal parameters of the entry. This includes the entry index (if any), the components (including discriminants) of the protected object, the Count attribute of an entry of that protected object, and data global to the protected unit.

The restriction against referencing the formal parameters within an [entry_barrier](S0192) ensures that all calls of the same entry see the same barrier value. If it is necessary to look at the parameters of an entry call before deciding whether to handle it, the [entry_barrier](S0192) can be "when True" and the caller can be requeued (on some private entry) when its parameters indicate that it cannot be handled immediately. 
...
The syntax rule for [entry_body](S0190) is new.

[Accept_statement](S0188)s can now have [exception_handler](S0232)s. 


### 9.5.3  Entry Calls

[An [entry_call_statement](S0194) (an entry call) can appear in various contexts.] A simple entry call is a stand-alone statement that represents an unconditional call on an entry of a target task or a protected object. [Entry calls can also appear as part of [select_statement](S0199)s (see 9.7).] 
...
The entry_[name](S0084) given in an [entry_call_statement](S0194) shall resolve to denote an entry. The rules for parameter associations are the same as for subprogram calls (see 6.4 and 6.4.1). 


#### Static Semantics

[The entry_[name](S0084) of an [entry_call_statement](S0194) specifies (explicitly or implicitly) the target object of the call, the entry or entry family, and the entry index, if any (see 9.5).] 
...
An entry of a task is open if the task is blocked on an [accept_statement](S0188) that corresponds to the entry (see 9.5.2), or on a [selective_accept](S0200) (see 9.7.1) with an open [accept_alternative](S0203) that corresponds to the entry; otherwise it is closed.

An entry of a protected object is open if the [condition](S0132) of the [entry_barrier](S0192) of the corresponding [entry_body](S0190) evaluates to True; otherwise it is closed. If the evaluation of the [condition](S0132) propagates an exception, the exception Program_Error is propagated to all current callers of all entries of the protected object. 
...
For the execution of an [entry_call_statement](S0194), evaluation of the [name](S0084) and of the parameter associations is as for a subprogram call (see 6.4). The entry call is then issued: For a call on an entry of a protected object, a new protected action is started on the object (see 9.5.1). The named entry is checked to see if it is open; if open, the entry call is said to be selected immediately, and the execution of the call proceeds as follows: 

For a call on an open entry of a task, the accepting task becomes ready and continues the execution of the corresponding [accept_statement](S0188) (see 9.5.2).

For a call on an open entry of a protected object, the corresponding [entry_body](S0190) is executed (see 9.5.2) as part of the protected action. 

If the [accept_statement](S0188) or [entry_body](S0190) completes other than by a requeue (see 9.5.4), return is made to the caller (after servicing the entry queues - see below); any necessary assigning back of formal to actual parameters occurs, as for a subprogram call (see 6.4.1); such assignments take place outside of any protected action. 
...
When the associated task reaches a corresponding [accept_statement](S0188), or a [selective_accept](S0200) with a corresponding open [accept_alternative](S0203);
...
If there is at least one call on a queue corresponding to an open entry, then one such call is selected according to the entry queuing policy in effect (see below), and the corresponding [accept_statement](S0188) or [entry_body](S0190) is executed as above for an entry call that is selected immediately.

The entry queuing policy controls selection among queued calls both for task and protected entry queues. The default entry queuing policy is to select calls on a given entry queue in order of arrival. If calls from two or more queues are simultaneously eligible for selection, the default entry queuing policy does not specify which queue is serviced first. Other entry queuing policies can be specified by [pragma](S0016)s (see D.4).
...
For an entry call that is added to a queue, and that is not the [triggering_statement](S0211) of an [asynchronous_select](S0209) (see 9.7.4), the calling task is blocked until the call is cancelled, or the call is selected and a corresponding [accept_statement](S0188) or [entry_body](S0190) completes without requeuing. In addition, the calling task is blocked during a rendezvous.
...
A call on an entry of a task that has already completed its execution raises the exception Tasking_Error at the point of the call; similarly, this exception is raised at the point of the call if the called task completes its execution or becomes abnormal before accepting the call or completing the rendezvous (see 9.8). This applies equally to a simple entry call and to an entry call as part of a [select_statement](S0199).


#### Implementation Permissions

An implementation may perform the sequence of steps of a protected action using any thread of control; it need not be that of the task that started the protected action. If an [entry_body](S0190) completes without requeuing, then the corresponding calling task may be made ready without waiting for the entire protected action to complete. 
...
When the entry of a protected object is checked to see whether it is open, the implementation need not reevaluate the [condition](S0132) of the corresponding [entry_barrier](S0192) if no variable or attribute referenced by the [condition](S0132) (directly or indirectly) has been altered by the execution (or cancellation) of a protected procedure or entry call on the object since the [condition](S0132) was last evaluated. 
...
An implementation may evaluate the [condition](S0132)s of all [entry_barrier](S0192)s of a given protected object any time any entry of the object is checked to see if it is open. 
...
NOTE 1   If an exception is raised during the execution of an [entry_body](S0190), it is propagated to the corresponding caller (see 11.4).
...
NOTE 4   The [condition](S0132) of an [entry_barrier](S0192) is allowed to be evaluated by an implementation more often than strictly necessary, even if the evaluation might have side effects. On the other hand, an implementation need not reevaluate the [condition](S0132) if nothing it references was updated by an intervening protected action on the protected object, even if the [condition](S0132) references some global variable that might have been updated by an action performed from outside of a protected action. 
...
The entry_[name](S0084) of a [requeue_statement](S0195) shall resolve to denote an entry (the target entry)that either has no parameters, or that has a profile that is type conformant (see 6.3.1) with the profile of the innermost enclosing [entry_body](S0190) or [accept_statement](S0188). 


#### Legality Rules

A [requeue_statement](S0195) shall be within a callable construct that is either an [entry_body](S0190) or an [accept_statement](S0188), and this construct shall be the innermost enclosing body or callable construct.
...
In a [requeue_statement](S0195) of an [accept_statement](S0188) of some task unit, either the target object shall be a part of a formal parameter of the [accept_statement](S0188), or the accessibility level of the target object shall not be equal to or statically deeper than any enclosing [accept_statement](S0188) of the task unit. In a [requeue_statement](S0195) of an [entry_body](S0190) of some protected unit, either the target object shall be a part of a formal parameter of the [entry_body](S0190), or the accessibility level of the target object shall not be statically deeper than that of the [entry_declaration](S0187).

Ramification: In the [entry_body](S0190) case, the intent is that the target object can be global, or can be a component of the protected unit, but cannot be a local variable of the [entry_body](S0190). 

Reason: These restrictions ensure that the target object of the requeue outlives the completion and finalization of the enclosing callable construct. They also prevent requeuing from a nested [accept_statement](S0188) on a parameter of an outer [accept_statement](S0188), which could create some strange "long-distance" connections between an entry caller and its server.

Note that in the strange case where a [task_body](S0179) is nested inside an [accept_statement](S0188), it is permissible to requeue from an [accept_statement](S0188) of the inner [task_body](S0179) on parameters of the outer [accept_statement](S0188). This is not a problem because all calls on the inner task have to complete before returning from the outer [accept_statement](S0188), meaning no "dangling calls" will be created. 

Implementation Note: By disallowing certain requeues, we ensure that the normal [terminate_alternative](S0205) rules remain sensible, and that explicit clearing of the entry queues of a protected object during finalization is rarely necessary. In particular, such clearing of the entry queues is necessary only (ignoring premature Unchecked_Deallocation) for protected objects declared in a [task_body](S0179) (or created by an allocator for an access type declared in such a body) containing one or more [requeue_statement](S0195)s. Protected objects declared in subprograms, or at the library level, will never need to have their entry queues explicitly cleared during finalization. 


#### Dynamic Semantics

The execution of a [requeue_statement](S0195) proceeds by first evaluating the entry_[name](S0084)[, including the [prefix](S0086) identifying the target task or protected object and the [expression](S0108) identifying the entry within an entry family, if any]. The [entry_body](S0190) or [accept_statement](S0188) enclosing the [requeue_statement](S0195) is then completed[, finalized, and left (see 7.6.1)].
...
If the new entry named in the [requeue_statement](S0195) has formal parameters, then during the execution of the [accept_statement](S0188) or [entry_body](S0190) corresponding to the new entry, the formal parameters denote the same objects as did the corresponding formal parameters of the callable construct completed by the requeue. [In any case, no parameters are specified in a [requeue_statement](S0195); any parameter passing is implicit.]

If the [requeue_statement](S0195) includes the reserved words with abort (it is a requeue-with-abort), then: 
...
If the reserved words with abort do not appear, then the call remains protected against cancellation while queued as the result of the [requeue_statement](S0195). 
...
NOTE 1   A requeue is permitted from a single entry to an entry of an entry family, or vice-versa. The entry index, if any, plays no part in the subtype conformance check between the profiles of the two entries; an entry index is part of the entry_[name](S0084) for an entry of a family. 
...
[ A [delay_statement](S0196) is used to block further execution until a specified expiration time is reached. The expiration time can be specified either as a particular point in time (in a [delay_until_statement](S0197)), or in seconds from the current time (in a [delay_relative_statement](S0198)). The language-defined package Calendar provides definitions for a type Time and associated operations, including a function Clock that returns the current time. ]
...
The expected type for the delay_[expression](S0108) in a [delay_relative_statement](S0198) is the predefined type Duration. The delay_[expression](S0108) in a [delay_until_statement](S0197) is expected to be of any nonlimited type.


#### Legality Rules

There can be multiple time bases, each with a corresponding clock, and a corresponding time type. The type of the delay_[expression](S0108) in a [delay_until_statement](S0197) shall be a time type - either the type Time defined in the language-defined package Calendar (see below), or some other implementation-defined time type (see D.8). 
...
For the execution of a [delay_statement](S0196), the delay_[expression](S0108) is first evaluated. For a [delay_until_statement](S0197), the expiration time for the delay is the value of the delay_[expression](S0108), in the time base associated with the type of the [expression](S0108). For a [delay_relative_statement](S0198), the expiration time is defined as the current time, in the time base associated with relative delays, plus the value of the delay_[expression](S0108) converted to the type Duration, and then rounded up to the next clock tick. The time base associated with relative delays is as defined in D.9, "Delay Accuracy" or is implementation defined. 
...
The task executing a [delay_statement](S0196) is blocked until the expiration time is reached, at which point it becomes ready again. If the expiration time has already passed, the task is not blocked. 

Discussion: For a [delay_relative_statement](S0198), this case corresponds to when the value of the delay_[expression](S0108) is zero or negative.

Even though the task is not blocked, it might be put back on the end of its ready queue. See D.2, "Priority Scheduling". 

If an attempt is made to cancel the [delay_statement](S0196) [(as part of an [asynchronous_select](S0209) or abort - see 9.7.4 and 9.8)], the _statement is cancelled if the expiration time has not yet passed, thereby completing the [delay_statement](S0196). 

Reason: This is worded this way so that in an [asynchronous_select](S0209) where the [triggering_statement](S0211) is a [delay_statement](S0196), an attempt to cancel the delay when the [abortable_part](S0212) completes is ignored if the expiration time has already passed, in which case the optional statements of the [triggering_alternative](S0210) are executed. 
...
An implementation may raise Time_Error if the value of a delay_[expression](S0108) in a [delay_until_statement](S0197) of a [select_statement](S0199) represents a time more than 90 days past the current time. The actual limit, if any, is implementation-defined. 

Implementation defined: Any limit on [delay_until_statement](S0197)s of [select_statement](S0199)s.

Implementation Note: This allows an implementation to implement [select_statement](S0199) timeouts using a representation that does not support the full range of a time type. In particular 90 days of seconds can be represented in 23 bits, allowing a signed 24-bit representation for the seconds part of a timeout. There is no similar restriction allowed for stand-alone [delay_until_statement](S0197)s, as these can be implemented internally using a loop if necessary to accommodate a long delay. 
...
The time base for [delay_relative_statement](S0198)s should be monotonic; it need not be the same time base as used for Calendar.Clock. 

NOTE 1   A [delay_relative_statement](S0198) with a negative value of the delay_[expression](S0108) is equivalent to one with a zero value.

NOTE 2   A [delay_statement](S0196) may be executed by the environment task; consequently [delay_statement](S0196)s may be executed as part of the elaboration of a [library_item](S0216) or the execution of the main subprogram. Such statements delay the environment task (see 10.2).

NOTE 3   A [delay_statement](S0196) is an abort completion point and a potentially blocking operation, even if the task is not actually blocked.
...
Ramification: The inaccuracy of the [delay_statement](S0196) has no relation to System.Tick. In particular, it is possible that the clock used for the [delay_statement](S0196) is less accurate than Calendar.Clock.
...
NOTE 5   Additional requirements associated with [delay_statement](S0196)s are given in D.9, "Delay Accuracy".
...
The syntax rule for [delay_statement](S0196) is modified to allow [delay_until_statement](S0197)s.
...
[There are four forms of the [select_statement](S0199). One form provides a selective wait for one or more [select_alternative](S0202)s. Two provide timed and conditional entry calls. The fourth provides asynchronous transfer of control.] 
...
[Asynchronous_select](S0209) is new. 


### 9.7.1  Selective Accept

[This form of the [select_statement](S0199) allows a combination of waiting for, and selecting from, one or more alternatives. The selection may depend on conditions associated with each alternative of the [selective_accept](S0200). ]
...
A [selective_accept](S0200) shall contain at least one [accept_alternative](S0203). In addition, it can contain: 

a [terminate_alternative](S0205) (only one); or

one or more [delay_alternative](S0204)s; or

an else part (the reserved word else followed by a [sequence_of_statements](S0123)). 
...
If a [selective_accept](S0200) contains more than one [delay_alternative](S0204), then all shall be [delay_relative_statement](S0198)s, or all shall be [delay_until_statement](S0197)s for the same time type. 
...
A [select_alternative](S0202) is said to be open if it is not immediately preceded by a [guard](S0201), or if the [condition](S0132) of its [guard](S0201) evaluates to True. It is said to be closed otherwise.

For the execution of a [selective_accept](S0200), any [guard](S0201) [condition](S0132)s are evaluated; open alternatives are thus determined. For an open [delay_alternative](S0204), the delay_[expression](S0108) is also evaluated. Similarly, for an open [accept_alternative](S0203) for an entry of a family, the [entry_index](S0189) is also evaluated. These evaluations are performed in an arbitrary order, except that a delay_[expression](S0108) or [entry_index](S0189) is not evaluated until after evaluating the corresponding [condition](S0132), if any. Selection and execution of one open alternative, or of the else part, then completes the execution of the [selective_accept](S0200); the rules for this selection are described below.

Open [accept_alternative](S0203)s are first considered. Selection of one such alternative takes place immediately if the corresponding entry already has queued calls. If several alternatives can thus be selected, one of them is selected according to the entry queuing policy in effect (see 9.5.3 and D.4). When such an alternative is selected, the selected call is removed from its entry queue and the [handled_sequence_of_statements](S0231) (if any) of the corresponding [accept_statement](S0188) is executed; after the rendezvous completes any subsequent [sequence_of_statements](S0123) of the alternative is executed. If no selection is immediately possible (in the above sense) and there is no else part, the task blocks until an open alternative can be selected.

Selection of the other forms of alternative or of an else part is performed as follows: 

An open [delay_alternative](S0204) is selected when its expiration time is reached if no [accept_alternative](S0203) or other [delay_alternative](S0204) can be selected prior to the expiration time. If several [delay_alternative](S0204)s have this same expiration time, one of them is selected according to the queuing policy in effect (see D.4); the default queuing policy chooses arbitrarily among the [delay_alternative](S0204)s whose expiration time has passed.

The else part is selected and its [sequence_of_statements](S0123) is executed if no [accept_alternative](S0203) can immediately be selected; in particular, if all alternatives are closed.

An open [terminate_alternative](S0205) is selected if the conditions stated at the end of clause 9.3 are satisfied. 

Ramification: In the absence of a [requeue_statement](S0195), the conditions stated are such that a [terminate_alternative](S0205) cannot be selected while there is a queued entry call for any entry of the task. In the presence of requeues from a task to one of its subtasks, it is possible that when a [terminate_alternative](S0205) of the subtask is selected, requeued calls (for closed entries only) might still be queued on some entry of the subtask. Tasking_Error will be propagated to such callers, as is usual when a task completes while queued callers remain. 

The exception Program_Error is raised if all alternatives are closed and there is no else part.

NOTE 1   A [selective_accept](S0200) is allowed to have several open [delay_alternative](S0204)s. A [selective_accept](S0200) is allowed to have several open [accept_alternative](S0203)s for the same entry.
...
The name of selective_wait was changed to [selective_accept](S0200) to better describe what is being waited for. We kept [select_alternative](S0202) as is, because selective_accept_alternative was too easily confused with [accept_alternative](S0203). 


### 9.7.2  Timed Entry Calls

[A [timed_entry_call](S0206) issues an entry call that is cancelled if the call (or a requeue-with-abort of the call) is not selected before the expiration time is reached. ]
...
For the execution of a [timed_entry_call](S0206), the entry_[name](S0084) and any actual parameters are evaluated, as for a simple entry call (see 9.5.3). The expiration time (see 9.6) for the call is determined by evaluating the delay_[expression](S0108) of the [delay_alternative](S0204); the entry call is then issued.

If the call is queued (including due to a requeue-with-abort), and not selected before the expiration time is reached, an attempt to cancel the call is made. If the call completes due to the cancellation, the optional [sequence_of_statements](S0123) of the [delay_alternative](S0204) is executed; if the entry call completes normally, the optional [sequence_of_statements](S0123) of the [entry_call_alternative](S0207) is executed. 

Ramification: The fact that the syntax calls for an [entry_call_statement](S0194) means that this fact is used in overload resolution. For example, if there is a procedure X and an entry X (both with no parameters), then "select X; ..." is legal, because overload resolution knows that the entry is the one that was meant. 
...
[A [conditional_entry_call](S0208) issues an entry call that is then cancelled if it is not selected immediately (or if a requeue-with-abort of the call is not selected immediately).] 
...
The execution of a [conditional_entry_call](S0208) is defined to be equivalent to the execution of a [timed_entry_call](S0206) with a [delay_alternative](S0204) specifying an immediate expiration time and the same [sequence_of_statements](S0123) as given after the reserved word else. 

NOTE 1   A [conditional_entry_call](S0208) may briefly increase the Count attribute of the entry, even if the conditional call is not selected.
...
[An asynchronous [select_statement](S0199) provides asynchronous transfer of control upon completion of an entry call or the expiration of a delay.] 
...
For the execution of an [asynchronous_select](S0209) whose [triggering_statement](S0211) is an [entry_call_statement](S0194), the entry_[name](S0084) and actual parameters are evaluated as for a simple entry call (see 9.5.3), and the entry call is issued. If the entry call is queued (or requeued-with-abort), then the [abortable_part](S0212) is executed. [If the entry call is selected immediately, and never requeued-with-abort, then the [abortable_part](S0212) is never started.]

For the execution of an [asynchronous_select](S0209) whose [triggering_statement](S0211) is a [delay_statement](S0196), the delay_[expression](S0108) is evaluated and the expiration time is determined, as for a normal [delay_statement](S0196). If the expiration time has not already passed, the [abortable_part](S0212) is executed.

If the [abortable_part](S0212) completes and is left prior to completion of the [triggering_statement](S0211), an attempt to cancel the [triggering_statement](S0211) is made. If the attempt to cancel succeeds (see 9.5.3 and 9.6), the [asynchronous_select](S0209) is complete.

If the [triggering_statement](S0211) completes other than due to cancellation, the [abortable_part](S0212) is aborted (if started but not yet completed - see 9.8). If the [triggering_statement](S0211) completes normally, the optional [sequence_of_statements](S0123) of the [triggering_alternative](S0210) is executed after the [abortable_part](S0212) is left. 

Discussion: We currently don't specify when the by-copy [in] out parameters are assigned back into the actuals. We considered requiring that to happen after the [abortable_part](S0212) is left. However, that doesn't seem useful enough to justify possibly overspecifying the implementation approach, since some of the parameters are passed by reference anyway.

In an earlier description, we required that the [sequence_of_statements](S0123) of the [triggering_alternative](S0210) execute after aborting the [abortable_part](S0212), but before waiting for it to complete and finalize, to provide more rapid response to the triggering event in case the finalization was unbounded. However, various reviewers felt that this created unnecessary complexity in the description, and a potential for undesirable concurrency (and nondeterminism) within a single task. We have now reverted to simpler, more deterministic semantics, but anticipate that further discussion of this issue might be appropriate during subsequent reviews. One possibility is to leave this area implementation defined, so as to encourage experimentation. The user would then have to assume the worst about what kinds of actions are appropriate for the [sequence_of_statements](S0123) of the [triggering_alternative](S0210) to achieve portability. 
...
[An [abort_statement](S0213) causes one or more tasks to become abnormal, thus preventing any further interaction with such tasks. The completion of the [triggering_statement](S0211) of an [asynchronous_select](S0209) causes a [sequence_of_statements](S0123) to be aborted.] 
...
Each task_[name](S0084) is expected to be of any task type[; they need not all be of the same task type.]


#### Dynamic Semantics

For the execution of an [abort_statement](S0213), the given task_[name](S0084)s are evaluated in an arbitrary order. Each named task is then aborted, which consists of making the task abnormal and aborting the execution of the corresponding [task_body](S0179), unless it is already completed. 

Ramification: Note that aborting those tasks is not defined to be an abort-deferred operation. Therefore, if one of the named tasks is the task executing the [abort_statement](S0213), or if the task executing the [abort_statement](S0213) depends on one of the named tasks, then it is possible for the execution of the [abort_statement](S0213) to be aborted, thus leaving some of the tasks unaborted. This allows the implementation to use either a sequence of calls to an "abort task" RTS primitive, or a single call to an "abort list of tasks" RTS primitive. 

When the execution of a construct is aborted (including that of a [task_body](S0179) or of a [sequence_of_statements](S0123)), the execution of every construct included within the aborted execution is also aborted, except for executions included within the execution of an abort-deferred operation; the execution of an abort-deferred operation continues to completion without being affected by the abort; the following are the abort-deferred operations: 
...
Ramification: Abort is deferred during the entire assignment operation to an object with a controlled part, even if only some subcomponents are controlled. Note that this says "assignment operation", not "[assignment_statement](S0130)". Explicit calls to Initialize, Finalize, or Adjust are not abort-deferred. 

When a master is aborted, all tasks that depend on that master are aborted.

The order in which tasks become abnormal as the result of an [abort_statement](S0213) or the abort of a [sequence_of_statements](S0123) is not specified by the language.

If the execution of an entry call is aborted, an immediate attempt is made to cancel the entry call (see 9.5.3). If the execution of a construct is aborted at a time when the execution is blocked, other than for an entry call, at a point that is outside the execution of an abort-deferred operation, then the execution of the construct completes immediately. For an abort due to an [abort_statement](S0213), these immediate effects occur before the execution of the [abort_statement](S0213) completes. Other than for these immediate cases, the execution of a construct that is aborted does not necessarily complete before the [abort_statement](S0213) completes. However, the execution of the aborted construct completes no later than its next abort completion point (if any) that occurs outside of an abort-deferred operation; the following are abort completion points for an execution: 

the point where the execution initiates the activation of another task;

the end of the activation of a task;

the start or end of the execution of an entry call, [accept_statement](S0188), [delay_statement](S0196), or [abort_statement](S0213); 

Ramification: Although the abort completion point doesn't occur until the end of the entry call or [delay_statement](S0196), these operations might be cut short because an abort attempts to cancel them. 

the start of the execution of a [select_statement](S0199), or of the [sequence_of_statements](S0123) of an [exception_handler](S0232). 

Reason: The start of an [exception_handler](S0232) is considered an abort completion point simply because it is easy for an implementation to check at such points. 
...
An attempt to execute an [asynchronous_select](S0209) as part of the execution of an abort-deferred operation is a bounded error. Similarly, an attempt to create a task that depends on a master that is included entirely within the execution of an abort-deferred operation is a bounded error. In both cases, Program_Error is raised if the error is detected by the implementation; otherwise the operations proceed as they would outside an abort-deferred operation, except that an abort of the [abortable_part](S0212) or the created task might or might not have an effect. 

Reason: An [asynchronous_select](S0209) relies on an abort of the [abortable_part](S0212) to effect the asynchronous transfer of control. For an [asynchronous_select](S0209) within an abort-deferred operation, the abort might have no effect.
...
NOTE 1   An [abort_statement](S0213) should be used only in situations requiring unconditional termination.
...
For a [prefix](S0086) T that is of a task type [(after any implicit dereference)], the following attributes are defined: 
...
NOTE 1   For the Count attribute, the entry can be either a single entry or an entry of a family. The name of the entry or entry family can be either a [direct_name](S0085) or an expanded name.

NOTE 2   Within task units, algorithms interrogating the attribute E'Count should take precautions to allow for the increase of the value of this attribute for incoming entry calls, and its decrease, for example with [timed_entry_call](S0206)s. Also, a [conditional_entry_call](S0208) may briefly increase this value, even if the conditional call is not accepted.

NOTE 3   Within protected units, algorithms interrogating the attribute E'Count in the [entry_barrier](S0192) for the entry E should take precautions to allow for the evaluation of the [condition](S0132) of the barrier both before and after queuing a given caller. 
...
If A1 is the action of issuing an entry call, and A2 is part of the corresponding execution of the appropriate [entry_body](S0190) or [accept_statement](S0188). 

Ramification: Evaluating the [entry_index](S0189) of an [accept_statement](S0188) is not synchronized with a corresponding entry call, nor is evaluating the entry barrier of an [entry_body](S0190). 

If A1 is part of the execution of an [accept_statement](S0188) or [entry_body](S0190), and A2 is the action of returning from the corresponding entry call;

If A1 is part of the execution of a protected procedure body or [entry_body](S0190) for a given protected object, and A2 is part of a later execution of an [entry_body](S0190) for the same protected object; 
...
A [pragma](S0016) Atomic or Atomic_Components may also be used to ensure that certain reads and updates are sequential - see C.6. 
