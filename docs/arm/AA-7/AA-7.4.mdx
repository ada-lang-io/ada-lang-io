...
{"{"}<em>{"AI05-0229-1"}</em>{"}"} {"{"}<em>{"AI05-0269-1"}</em>{"}"} <br />The first part of the last sentence is redundant, as no imported entity may have a completion, as stated in B.1. <br /></Admonition>
...
<p>{"{"}<em>{"AI95-00256-01"}</em>{"}"} <br />The completion of a deferred constant declaration shall occur before the constant is frozen (see 13.14).<br /></p>
...
<Admonition type="aarm" aarm="note" title="Note: ">
{"{"}<em>{"AI05-0299-1"}</em>{"}"} <br />Since deferred constants can now be of a nonprivate type, we have made this a stand-alone subclause, rather than a subclause of 7.3, "Private Types and Private Extensions".<br /></Admonition>
