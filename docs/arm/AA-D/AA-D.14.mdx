...
<p>{"{"}<em>{"AI95-00307-01"}</em>{"}"} <br />The type Timer represents an execution-time event for a single task and is capable of detecting execution-time overruns. The access discriminant T identifies the task concerned. The type Timer needs finalization (see <a href="../AA-7/AA-7.6">7.6</a>).<br /></p>
...
<p>{"{"}<em>{"AI95-00354-01"}</em>{"}"} <br />The type Group_Budget represents an execution time budget to be used by a group of tasks. The type Group_Budget needs finalization (see <a href="../AA-7/AA-7.6">7.6</a>). A task can belong to at most one group. Tasks of any priority can be added to a group.<br /></p>
