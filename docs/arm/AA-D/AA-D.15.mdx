...
<MarginInfo items={["AI95-00297-01"]} />
<p>If the Ceiling{"_"}Locking policy (see <a href="/docs/arm/AA-D/AA-D.3">D.3</a>) is in effect when a procedure Set{"_"}Handler is called, a check is made that the ceiling priority of Handler.<strong>all</strong> is Interrupt{"_"}Priority'Last. If the check fails, Program{"_"}Error is raised.<br /></p>
