...
<p><strong></strong>By-reference parameter passing makes sense only if there is an object to reference; hence, we define such an object for each case.</p></Admonition>
...
<Admonition type="aarm" aarm="reason" title="Reason: ">
<p><strong></strong>We do not want to go so far as to say that the mere presence of aliasing is wrong. We wish to be able to write the following sorts of things in standard Ada: </p></Admonition>
...
<p>{"{"}<em>{"AI05-0096-1"}</em>{"}"} <strong></strong> Corrected so that limited derived types are by-reference only if their parent is.</p></Admonition>
