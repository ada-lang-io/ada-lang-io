...
<p>{"{"}<em>{"AI05-0248-1"}</em>{"}"} The rules guarantee that remote call interface and shared passive library units are consistent among all partitions prior to the execution of a distributed program, so that the semantics of the distributed program are well defined.</p>

#### Static Semantics

<p>For a <code><a href="./AA-4.1#S0093">prefix</a></code> P that statically denotes a program unit, the following attributes are defined: </p>
