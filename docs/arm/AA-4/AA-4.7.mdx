...
<p><code>qualified_expression</code><a id="S0163"></a><code> ::= </code><br />   <code><a href="../AA-3/AA-3.2#S0028">subtype_mark</a></code>'(<code><a href="../AA-4/AA-4.4#S0132">expression</a></code>) | <code><a href="../AA-3/AA-3.2#S0028">subtype_mark</a></code>'<code><a href="../AA-4/AA-4.3#S0106">aggregate</a></code><br /></p>
...
<strong></strong>{"{"}<em>{"AI05-0003-1"}</em>{"}"} This is stated in 3.3. <br /></Admonition>
...
{"{"}<em>{"AI12-0100-1"}</em>{"}"} The effect of a failed predicate check is as defined in 3.2.4; such a check could raise any exception, not just Constraint_Error or Assertion_Error. <br /></Admonition>
...
<Admonition type="aarm" aarm="note" title="Note: ">
{"{"}<em>{"AI05-0003-1"}</em>{"}"} Added a definition of the nominal subtype of a <code><a href="../AA-4/AA-4.7#S0163">qualified_expression</a></code>. <br /></Admonition>


#### Inconsistencies With Ada 2012

<Admonition type="aarm" aarm="note" title="Note: ">
{"{"}<em>{"AI12-0100-1"}</em>{"}"} {"{"}<em>{"AI12-0005-1"}</em>{"}"} <strong>Corrigendum:</strong> A <code><a href="../AA-4/AA-4.7#S0163">qualified_expression</a></code> now performs a predicate check for the named subtype (if it is enabled). Original Ada 2012 did not include that check (an omission). While this is formally inconsistent (an exception could be raised when none would be raised by original Ada 2012), cases where this scenario arises are likely to be rare (the qualified expression would have to have a stricter subtype than the following usage) and the check is more likely to detect bugs than be unexpected. <br /></Admonition>


#### Wording Changes from Ada 2012

<Admonition type="aarm" aarm="note" title="Note: ">
{"{"}<em>{"AI12-0325-1"}</em>{"}"} Reworded the resolution rule so that the operand of a <code><a href="../AA-4/AA-4.7#S0163">qualified_expression</a></code> has an expected type. This eliminates an annoying inconsistency in the language definition. <br /></Admonition>
