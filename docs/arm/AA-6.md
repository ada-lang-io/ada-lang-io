...
:::
A subprogram is a program unit or intrinsic operation whose execution is invoked by a subprogram call. There are two forms of subprogram: procedures and functions. A procedure call is a [statement](S0124); a function call is an expression and returns a value. The definition of a subprogram can be given in two parts: a subprogram declaration defining its interface, and a [subprogram_body](S0154) defining its execution. [Operators and enumeration literals are functions.] 

To be honest: A function call is an expression, but more specifically it is a [name](S0084). 
...
A callable entity is a subprogram or entry (see 9.5.2). A callable entity is invoked by a call; that is, a subprogram call or entry call. A callable construct is a construct that defines the action of a call upon a callable entity: a [subprogram_body](S0154), [entry_body](S0190), or [accept_statement](S0188). 
...
[A [subprogram_declaration](S0141) declares a procedure or function.] 
...
[The optional [parent_unit_name](S0220) is only allowed for library units (see 10.1.1).] 

operator_symbol ::= [string_literal](S0013)

The sequence of characters in an [operator_symbol](S0147) shall correspond to an operator belonging to one of the six classes of operators defined in clause 4.5(spaces are not allowed and the case of letters is not significant).
...
A formal parameter is an object [directly visible within a [subprogram_body](S0154)] that represents the actual parameter passed to the subprogram in a call; it is declared by a [parameter_specification](S0152). For a formal parameter, the expected type for its [default_expression](S0060), if any, is that of the formal parameter. 


#### Legality Rules

The parameter mode of a formal parameter conveys the direction of information transfer with the actual parameter: in, in out, or out. Mode in is the default, and is the mode of a parameter defined by an [access_definition](S0077). The formal parameters of a function, if any, shall have the mode in. 
...
A [default_expression](S0060) is only allowed in a [parameter_specification](S0152) for a formal parameter of mode in.

A [subprogram_declaration](S0141) or a [generic_subprogram_declaration](S0237) requires a completion: [a body, a [renaming_declaration](S0169) (see 8.5), or a pragma Import (see B.1)]. [A completion is not allowed for an [abstract_subprogram_declaration](S0142).] 

Ramification: Abstract subprograms are not declared by [subprogram_declaration](S0141)s, and so do not require completion. Protected subprograms are declared by [subprogram_declaration](S0141)s, and so require completion. Note that an abstract subprogram is a subprogram, and a protected subprogram is a subprogram, but a generic subprogram is not a subprogram. 

A [name](S0084) that denotes a formal parameter is not allowed within the [formal_part](S0151) in which it is declared, nor within the [formal_part](S0151) of a corresponding body or [accept_statement](S0188). 

Ramification: By contrast, [generic_formal_parameter_declaration](S0240)s are visible to subsequent declarations in the same [generic_formal_part](S0239). 


#### Static Semantics

The profile of (a view of) a callable entity is either a [parameter_profile](S0149) or [parameter_and_result_profile](S0150)[; it embodies information about the interface to that entity - for example, the profile includes information about parameters passed to the callable entity. All callable entities have a profile - enumeration literals, other subprograms, and entries. An access-to-subprogram type has a designated profile.] Associated with a profile is a calling convention. A [subprogram_declaration](S0141) declares a procedure or a function, as indicated by the initial reserved word, with name and profile as given by its specification.

The nominal subtype of a formal parameter is the subtype denoted by the [subtype_mark](S0025), or defined by the [access_definition](S0077), in the [parameter_specification](S0152).

An access parameter is a formal in parameter specified by an [access_definition](S0077). An access parameter is of an anonymous general access-to-variable type (see 3.10). [Access parameters  allow dispatching calls to be controlled by access values.]
...
[A subprogram declared by an [abstract_subprogram_declaration](S0142) is abstract; a subprogram declared by a [subprogram_declaration](S0141) is not. See 3.9.3, "Abstract Types and Subprograms".]


#### Dynamic Semantics

The elaboration of a [subprogram_declaration](S0141) or an [abstract_subprogram_declaration](S0142) has no effect. 

NOTE 1   A [parameter_specification](S0152) with several identifiers is equivalent to a sequence of single [parameter_specification](S0152)s, as explained in 3.3.
...
NOTE 3   The evaluation of [default_expression](S0060)s is caused by certain calls, as described in 6.4.1. They are not evaluated during the elaboration of the subprogram declaration.
...
The syntax for [abstract_subprogram_declaration](S0142) is added. The syntax for [parameter_specification](S0152) is revised to allow for access parameters (see 3.10)

Program units that are library units may have a [parent_unit_name](S0220) to indicate the parent of a child (see Section 10). 
...
The syntax rule for [defining_operator_symbol](S0148) is new. It is used for the defining occurrence of an [operator_symbol](S0147), analogously to [defining_identifier](S0019). Usage occurrences use the [direct_name](S0085) or [selector_name](S0092) syntactic categories. The syntax rules for [defining_designator](S0145) and [defining_program_unit_name](S0146) are new. 
...
[A [parameter_specification](S0152) declares a formal parameter of mode in, in out, or out.] 
...
A parameter of a by-reference type is passed by reference. Each value of a by-reference type has an associated object. For a parenthesized expression, [qualified_expression](S0121), or [type_conversion](S0120), this object is the one associated with the operand.
...
For an [assignment_statement](S0130) assigning one array parameter to another, the implementation has to check which direction to copy at run time, in general, in case the actual parameters are overlapping slices. For example: 
...
NOTE 1   A formal parameter of mode in is a constant view (see 3.3); it cannot be updated within the [subprogram_body](S0154).
...
[A [subprogram_body](S0154) specifies the execution of a subprogram.] 
...
If a [designator](S0144) appears at the end of a [subprogram_body](S0154), it shall repeat the [defining_designator](S0145) of the [subprogram_specification](S0143). 


#### Legality Rules

[In contrast to other bodies,] a [subprogram_body](S0154) need not be the completion of a previous declaration[, in which case the body declares the subprogram]. If the body is a completion, it shall be the completion of a [subprogram_declaration](S0141) or [generic_subprogram_declaration](S0237). The profile of a [subprogram_body](S0154) that completes a declaration shall conform fully to that of the declaration. 


#### Static Semantics

A [subprogram_body](S0154) is considered a declaration. It can either complete a previous declaration, or itself be the initial declaration of the subprogram. 


#### Dynamic Semantics

The elaboration of a nongeneric [subprogram_body](S0154) has no other effect than to establish that the subprogram can from then on be called without failing the Elaboration_Check. 

Ramification: See 12.2 for elaboration of a generic body. Note that protected subprogram_bodies never get elaborated; the elaboration of the containing [protected_body](S0185) allows them to be called without failing the Elaboration_Check. 

[The execution of a [subprogram_body](S0154) is invoked by a subprogram call.] For this execution the [declarative_part](S0079) is elaborated, and the [handled_sequence_of_statements](S0231) is then executed. 
...
A [renaming_declaration](S0169) may be used instead of a [subprogram_body](S0154). 


#### Wording Changes from Ada 83

The syntax rule for [subprogram_body](S0154) now uses the syntactic category [handled_sequence_of_statements](S0231).

The [declarative_part](S0079) of a [subprogram_body](S0154) is now required; that doesn't make any real difference, because a [declarative_part](S0079) can be empty.

We have incorporated some rules from RM83-6.5 here.

RM83 forgot to restrict the definition of elaboration of a [subprogram_body](S0154) to nongenerics. 
...
The default calling convention for any subprogram not listed below is Ada. [A [pragma](S0016) Convention, Import, or Export may be used to override the default calling convention (see B.1)]. 
...
a subprogram declared immediately within a [protected_body](S0185).
...
Of these four conventions, only Ada and Intrinsic are allowed as a convention_[identifier](S0002) in a [pragma](S0016) Convention, Import, or Export. 
...
Two profiles are fully conformant if they are subtype-conformant, and corresponding parameters have the same names and have [default_expression](S0060)s that are fully conformant with one another. 
...
Two expressions are fully conformant if, [after replacing each use of an operator with the equivalent [function_call](S0156):] 

each constituent construct of one corresponds to an instance of the same syntactic category in the other, except that an expanded name may correspond to a [direct_name](S0085) (or [character_literal](S0012)) or to a different expanded name in the other; and

each [direct_name](S0085), [character_literal](S0012), and [selector_name](S0092) that is not part of the [prefix](S0086) of an expanded name in one denotes the same declaration as the corresponding [direct_name](S0085), [character_literal](S0012), or [selector_name](S0092) in the other; and 

Ramification: Note that it doesn't say "respectively" because a [direct_name](S0085) can correspond to a [selector_name](S0092), and vice-versa, by the previous bullet. This rule allows the [prefix](S0086) of an expanded name to be removed, or replaced with a different [prefix](S0086) that denotes a renaming of the same entity. However, it does not allow a [direct_name](S0085) or [selector_name](S0092) to be replaced with one denoting a distinct renaming (except for [direct_name](S0085)s and [selector_name](S0092)s in [prefix](S0086)es of expanded names). Note that calls using operator notation are equivalent to calls using prefix notation.
...
Within Main, the expressions "F", "A.F", "B.A_View.F", and "A_View.F" are all fully conformant with one another. However, "F" and "F_View" are not fully conformant. If they were, it would be bad news, since the two denoted views have different [default_expression](S0060)s. 

each [primary](S0113) that is a literal in one has the same value as the corresponding literal in the other.
...
Two [known_discriminant_part](S0058)s are fully conformant if they have the same number of discriminants, and discriminants in the same positions have the same names, statically matching subtypes, and [default_expression](S0060)s that are fully conformant with one another. 

Two [discrete_subtype_definition](S0052)s are fully conformant if they are both [subtype_indication](S0024)s or are both [range](S0034)s, the [subtype_mark](S0025)s (if any) denote the same subtype, and the corresponding [simple_expression](S0110)s of the [range](S0034)s (if any) fully conform. 

Ramification: In the [subtype_indication](S0024) case, any ranges have to be corresponding; that is, two [subtype_indication](S0024)s cannot conform unless both or neither has a [range](S0034). 

Discussion: This definition is used in 9.5.2, "Entries and Accept Statements" for the conformance required between the [discrete_subtype_definition](S0052)s of an [entry_declaration](S0187) for a family of entries and the corresponding [entry_index_specification](S0193) of the [entry_body](S0190). 
...
The form of a [pragma](S0016) Inline, which is a program unit pragma (see 10.1.5), is as follows: 

  pragma Inline([name](S0084) {, [name](S0084)}); 


#### Legality Rules

The [pragma](S0016) shall apply to one or more callable entities or generic subprograms. 


#### Static Semantics

If a [pragma](S0016) Inline applies to a callable entity, this indicates that inline expansion is desired for all calls to that entity. If a [pragma](S0016) Inline applies to a generic subprogram, this indicates that inline expansion is desired for all calls to all instances of that generic subprogram.
...
Ramification: The meaning of a subprogram can be changed by a [pragma](S0016) Inline only in the presence of failing checks (see 11.6). 


#### Implementation Permissions

For each call, an implementation is free to follow or to ignore the recommendation expressed by the [pragma](S0016). 
...
NOTE 1   The [name](S0084) in a [pragma](S0016) Inline can denote more than one entity in the case of overloading. Such a [pragma](S0016) applies to all of the denoted entities. 


#### Extensions to Ada 83

A [pragma](S0016) Inline is allowed inside a [subprogram_body](S0154) if there is no corresponding [subprogram_declaration](S0141). This is for uniformity with other program unit pragmas. 


## 6.4  Subprogram Calls

A subprogram call is either a [procedure_call_statement](S0155) or a [function_call](S0156); [it invokes the execution of the [subprogram_body](S0154). The call specifies the association of the actual parameters, if any, with formal parameters of the subprogram.] 
...
A [parameter_association](S0158) is named or positional according to whether or not the formal_parameter_[selector_name](S0092) is specified. Any positional associations shall precede any named associations. Named associations are not allowed if the [prefix](S0086) in a subprogram call is an [attribute_reference](S0093). 
...
The [name](S0084) or [prefix](S0086) given in a [procedure_call_statement](S0155) shall resolve to denote a callable entity that is a procedure, or an entry renamed as (viewed as) a procedure. The [name](S0084) or [prefix](S0086) given in a [function_call](S0156) shall resolve to denote a callable entity that is a function. [When there is an [actual_parameter_part](S0157), the [prefix](S0086) can be an [implicit_dereference](S0088) of an access-to-subprogram value.] 

Ramification: The function can be an operator, enumeration literal, attribute that is a function, etc. 

A subprogram call shall contain at most one association for each formal parameter. Each formal parameter without an association shall have a [default_expression](S0060) (in the profile of the view denoted by the [name](S0084) or [prefix](S0086)). [This rule is an overloading rule (see 8.6).] 


#### Dynamic Semantics

For the execution of a subprogram call, the [name](S0084) or [prefix](S0086) of the call is evaluated, and each [parameter_association](S0158) is evaluated (see 6.4.1). If a [default_expression](S0060) is used, an implicit [parameter_association](S0158) is assumed for this rule. These evaluations are done in an arbitrary order. The [subprogram_body](S0154) is then executed. Finally, if the subprogram completes normally, then after it is left, any necessary assigning back of formal to actual parameters occurs (see 6.4.1).
...
Normally, the [subprogram_body](S0154) that is executed by the above rule is the one for the subprogram being called. For an enumeration literal, implicitly declared (but noninherited) subprogram, or an attribute that is a subprogram, an implicit body is assumed. For a dispatching call, 3.9.2, "Dispatching Operations of Tagged Types" defines which [subprogram_body](S0154) is executed. 

The exception Program_Error is raised at the point of a [function_call](S0156) if the function completes normally without executing a [return_statement](S0160). 
...
A [function_call](S0156) denotes a constant, as defined in 6.5; the nominal subtype of the constant is given by the result subtype of the function. 
...
NOTE 1   If a [default_expression](S0060) is used for two or more parameters in a multiple [parameter_specification](S0152), the [default_expression](S0060) is evaluated once for each omitted parameter. Hence in the above examples, the two calls of Pair are equivalent. 
...
We have gotten rid of parameters "of the form of a type conversion" (see RM83-6.4.1(3)). The new view semantics of [type_conversion](S0120)s allows us to use normal [type_conversion](S0120)s instead.
...
The formal_parameter_[selector_name](S0092) of a [parameter_association](S0158) shall resolve to denote a [parameter_specification](S0152) of the view being called.

The actual parameter is either the [explicit_actual_parameter](S0159) given in a [parameter_association](S0158) for a given formal parameter, or the corresponding [default_expression](S0060) if no [parameter_association](S0158) is given for the formal parameter. The expected type for an actual parameter is the type of the corresponding formal parameter. 

To be honest: The corresponding [default_expression](S0060) is the one of the corresponding formal parameter in the profile of the view denoted by the [name](S0084) or [prefix](S0086) of the call. 

If the mode is in, the actual is interpreted as an [expression](S0108); otherwise, the actual is interpreted only as a [name](S0084), if possible. 

Ramification: This formally resolves the ambiguity present in the syntax rule for [explicit_actual_parameter](S0159).  Note that we don't actually require that the actual be a [name](S0084) if the mode is not in; we do that below.


#### Legality Rules

If the mode is in out or out, the actual shall be a [name](S0084) that denotes a variable. 

Discussion: We no longer need "or a [type_conversion](S0120) whose argument is the [name](S0084) of a variable", because a [type_conversion](S0120) is now a [name](S0084), and a [type_conversion](S0120) of a variable is a variable. 

Reason: The requirement that the actual be a (variable) [name](S0084) is not an overload resolution rule, since we don't want the difference between [expression](S0108) and [name](S0084) to be used to resolve overloading. For example: 
...
The above call to Print is ambiguous even though the call is not compatible with the second Print which requires an actual that is a (variable) [name](S0084) ("3" is an [expression](S0108), not a [name](S0084)). This requirement is a legality rule, so overload resolution fails before it is considered, meaning that the call is ambiguous. 
...
For the evaluation of a [parameter_association](S0158): 

The actual parameter is first evaluated.

For an access parameter, the [access_definition](S0077) is elaborated, which creates the anonymous access type.
...
Ramification: This includes an array type whose component type is an access type, and a record type with a component that has a [default_expression](S0060), among other things. 
...
Ramification: If any conversion or assignment as part of parameter passing propagates an exception, the exception is raised at the place of the subprogram call; that is, it cannot be handled inside the [subprogram_body](S0154). 

Proof: Since these checks happen before or after executing the [subprogram_body](S0154), the execution of the [subprogram_body](S0154) does not dynamically enclose them, so it can't handle the exceptions. 

Discussion: The variable we're talking about is the one denoted by the variable_[name](S0084) given as the [explicit_actual_parameter](S0159). If this variable_[name](S0084) is a [type_conversion](S0120), then the rules in 4.6 for assigning to a view conversion apply. That is, if X is of subtype S1, and the actual is S2(X), the above-mentioned conversion will convert to S2, and the one mentioned in 4.6 will convert to S1. 
...
A [return_statement](S0160) is used to complete the execution of the innermost enclosing [subprogram_body](S0154), [entry_body](S0190), or [accept_statement](S0188). 
...
The [expression](S0108), if any, of a [return_statement](S0160) is called the return expression. The result subtype of a function is the subtype denoted by the [subtype_mark](S0025) after the reserved word return in the profile of the function. The expected type for a return expression is the result type of the corresponding function. 
...
A [return_statement](S0160) shall be within a callable construct, and it applies to the innermost one. A [return_statement](S0160) shall not be within a body that is within the construct to which the [return_statement](S0160) applies.

A function body shall contain at least one [return_statement](S0160) that applies to the function body, unless the function contains [code_statement](S0273)s. A [return_statement](S0160) shall include a return expression if and only if it applies to a function body.

Reason: The requirement that a function body has to have at least one [return_statement](S0160) is a "helpful" restriction. There was been some interest in lifting this restriction, or allowing a raise statement to substitute for the [return_statement](S0160). However, there was enough interest in leaving it as is that we decided not to change it. 


#### Dynamic Semantics

For the execution of a [return_statement](S0160), the [expression](S0108) (if any) is first evaluated and converted to the result subtype. 

Ramification: The conversion might raise Constraint_Error - (see 4.6). 

If the result type is class-wide, then the tag of the result is the tag of the value of the [expression](S0108). 
...
a [name](S0084) that denotes an object view whose accessibility level is not deeper than that of the master that elaborated the function body; or 
...
a parenthesized expression or [qualified_expression](S0121) whose operand is one of these kinds of expressions.
...
Finally, a transfer of control is performed which completes the execution of the callable construct to which the [return_statement](S0160) applies, and returns to the caller.
...
```ada
return;                         -- in a procedure body, [entry_body](S0190), or [accept_statement](S0188)
...
We have clarified that a [return_statement](S0160) applies to a callable construct, not to a callable entity.

There is no need to mention generics in the rules about where a [return_statement](S0160) can appear and what it applies to; the phrase "body of a subprogram or generic subprogram" is syntactic, and refers exactly to "[subprogram_body](S0154)". 
...
An operator is a function whose [designator](S0144) is an [operator_symbol](S0147). [Operators, like other functions, may be overloaded.] 


#### Name Resolution Rules

Each use of a unary or binary operator is equivalent to a [function_call](S0156) with function_[prefix](S0086) being the corresponding [operator_symbol](S0147), and with (respectively) one or two positional actual parameters being the operand(s) of the operator (in order). 

To be honest: We also use the term operator (in Section 4 and in 6.1) to refer to one of the syntactic categories defined in 4.5, "Operators and Expression Evaluation" whose names end with "_operator:" [logical_operator](S0114), [relational_operator](S0115), [binary_adding_operator](S0116), [unary_adding_operator](S0117), [multiplying_operator](S0118), and [highest_precedence_operator](S0119). 


#### Legality Rules

The [subprogram_specification](S0143) of a unary or binary operator shall have one or two parameters, respectively. A generic function instantiation whose [designator](S0144) is an [operator_symbol](S0147) is only allowed if the specification of the generic function has the corresponding number of parameters.

[Default_expression](S0060)s are not allowed for the parameters of an operator (whether the operator is declared with an explicit [subprogram_specification](S0143) or by a [generic_instantiation](S0241)).
