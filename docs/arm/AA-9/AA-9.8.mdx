...
<Admonition type="aarm" aarm="ramification" title="Ramification: ">
<p><strong></strong>Although the abort completion point doesn't occur until the end of the entry call or <code><a href="../AA-9/AA-9.6#S0266">delay_statement</a></code>, these operations might be cut short because an abort attempts to cancel them. </p></Admonition>
...
<Admonition type="aarm" aarm="reason" title="Reason: ">
<p><strong></strong>The start of an <code><a href="../AA-11/AA-11.2#S0305">exception_handler</a></code> is considered an abort completion point simply because it is easy for an implementation to check at such points. </p></Admonition>

<Admonition type="aarm" aarm="implementation-note" title="Implementation Note: ">
<p><strong></strong>Implementations may of course check for abort more often than at each abort completion point; ideally, a fully preemptive implementation of abort will be provided. If preemptive abort is not supported in a given environment, then supporting the checking for abort as part of subprogram calls and loop iterations might be a useful option. </p></Admonition>
