...
<Admonition type="aarm" aarm="reason" title="Reason: ">
<strong></strong>{"{"}<em>{"AI12-0005-1"}</em>{"}"} {"{"}<em>{"AI12-0119-1"}</em>{"}"} <br />Any two actions of the same logical thread of control are sequential, even if one does not signal the other because they can be executed in an "arbitrary" (but necessarily equivalent to some "sequential") order. <br /></Admonition>
...
<p>{"{"}<em>{"AI05-0229-1"}</em>{"}"} <br />Aspect Atomic or aspect Atomic_Components may also be specified to ensure that certain reads and updates are sequential - see <a href="../AA-C/AA-C.6">C.6</a>. <br /></p>
