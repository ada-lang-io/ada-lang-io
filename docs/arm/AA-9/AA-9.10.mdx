...
<li>{"{"}<em>8652/0031</em>{"}"} {"{"}<em>{"AI95-00118-01"}</em>{"}"} {"{"}<em>{"AI05-0072-1"}</em>{"}"} <br />If A1 is the termination of a task T, and A2 is either an evaluation of the expression T'Terminated that results in True, or a call to Ada.Task_Identification.Is_Terminated with an actual parameter that identifies T and a result of True (see C.7.1);<br /></li>
...
<p>{"{"}<em>{"AI05-0229-1"}</em>{"}"} <br />Aspect Atomic or aspect Atomic_Components may also be specified to ensure that certain reads and updates are sequential - see C.6. <br /></p>
...
<p>{"{"}<em>{"AI12-0267-1"}</em>{"}"} <br />This subclause determines what checks are performed relating to possible concurrent conflicting actions (see 9.10). <br /></p>
