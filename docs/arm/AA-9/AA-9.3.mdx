...
<p><strong></strong>{"{"}<em>{"AI95-00416-01"}</em>{"}"} The master of a task created by a return statement changes when the accessibility of the return object changes. Note that its activation happens, if at all, only after the function returns and all accessibility level changes have occurred. </p></Admonition>
...
<p><strong></strong>Don't confuse these kinds of dependences with the dependences among compilation units defined in 10.1.1, "Compilation Units - Library Units". </p></Admonition>
...
<Admonition type="aarm" aarm="ramification" title="Ramification: ">
<p><strong></strong>Any required finalization is performed after the selection of <code><a href="../AA-9/AA-9.7#S0275">terminate_alternative</a></code>s. The tasks are not callable during the finalization. In some ways it is as though they were aborted. </p></Admonition>
