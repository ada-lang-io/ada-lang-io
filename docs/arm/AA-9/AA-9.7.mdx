...
<p><strong></strong>This simplifies the implementation and the description of the semantics. </p></Admonition>
...
<p><strong></strong>In the absence of a <code><a href="../AA-9/AA-9.5#S0265">requeue_statement</a></code>, the conditions stated are such that a <code><a href="../AA-9/AA-9.7#S0275">terminate_alternative</a></code> cannot be selected while there is a queued entry call for any entry of the task. In the presence of requeues from a task to one of its subtasks, it is possible that when a <code><a href="../AA-9/AA-9.7#S0275">terminate_alternative</a></code> of the subtask is selected, requeued calls (for closed entries only) might still be queued on some entry of the subtask. Tasking_Error will be propagated to such callers, as is usual when a task completes while queued callers remain. </p></Admonition>
