...
<p>{"{"}<em>8652/0009</em>{"}"} {"{"}<em>{"AI95-00137-01"}</em>{"}"} <br /><code>basic_declarative_item</code><a id="S0088"></a><code> ::= </code><br />    <code><a href="../AA-3/AA-3.1#S0021">basic_declaration</a></code> | <code><a href="../AA-13/AA-13.1#S0343">aspect_clause</a></code> | <code><a href="../AA-8/AA-8.4#S0235">use_clause</a></code><br /></p>
...
<ul>
<li>{"{"}<em>{"AI05-0229-1"}</em>{"}"} <br />For a call to a protected operation of a protected type (that has a body - no check is performed if  the protected type is imported - see B.1), a check is made that the <code><a href="../AA-9/AA-9.4#S0254">protected_body</a></code> is already elaborated. This check and the evaluations of any actual parameters of the call are done in an arbitrary order. <br /></li>
</ul>
<Admonition type="aarm" aarm="discussion" title="Discussion: ">
...
<p>{"{"}<em>{"AI05-0229-1"}</em>{"}"} <br />An implicit declaration shall not have a completion. For any explicit declaration that is specified to <em>require completion</em>, there shall be a corresponding explicit completion, unless the declared entity is imported (see B.1). <br /></p>
