...
<strong></strong>{"{"}<em>{"AI12-0005-1"}</em>{"}"} <br />The logical threads of control associated with the execution of a given parallel construct all execute as part of the execution of one task (see <a href="../AA-9/">9</a>, "<a href="../AA-9/">Tasks and Synchronization</a>"). Thus, the result returned by a call to Task_Identification.Current_Task is independent of whether the call takes place during the execution of a parallel construct. <br /></Admonition>
...
<p>{"{"}<em>{"AI95-00266-02"}</em>{"}"} <br />As part of the finalization of a <code><a href="../AA-9/AA-9.1#S0248">task_body</a></code>, after performing the actions specified in <a href="../AA-7/AA-7.6">7.6</a> for finalization of a master, the specific handler for the task, if one is set, is executed. If the specific handler is cleared, a search for a fall-back handler proceeds by recursively following the master relationship for the task. If a task is found whose fall-back handler is set, that handler is executed; otherwise, no handler is executed.<br /></p>
