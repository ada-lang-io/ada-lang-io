...
<p><strong>Implementation Advice: </strong>If COBOL (respectively, C) is supported in the target environment, then interfacing to COBOL (respectively, C) should be supported as specified in Annex B.</p></Admonition>
...
<Admonition type="note">
<p>{"{"}<em>{"AI95-00285-01"}</em>{"}"} Added a mention of Wide_Wide_Text_IO.Editing, part of the support for 32-bit characters. </p></Admonition>
