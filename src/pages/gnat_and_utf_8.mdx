...
:::note To be investigated
 ...
 Shouldn't the compiler raise compilation error on
 non-printable characters in a string literal??
 
:::

You can tell the compiler the encoding of the input texts by passing
the `-gnatW8` option. Here is the described in the options list
(`gnatmake --help`):
...
were not deprecated at all. Moreover, `String` is widely used in the standard
...
The guide is outdated here, since current Ada Standards defines the
*identifier* lexical element in Unicode terms, not in Latin-1
characters at all.
...
I don't understand what incompatibility could arise. I've tried to
compile
...
and corresponding code in "brackets encoding". There are no differences.
